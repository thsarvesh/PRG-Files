...
if type('FINALCP')='U'
	...
	datext= 'DBF'
ENDIF
datatransferpath=Subs('D:\DM\Apr\Pharmasodr\data\'+Space(100),1,100)
datatransferext='DBF'
datatransfersql='No '
datatransfermdb='No '
old_3 = 'YES'
...
Do While .Not. Eof()
	...
	*Sundry Creditors
	If fact.accmst_cld = 'Supplier               '  .Or. fact.accmst_cld = 'Customer-Supplier      '
...
Wait Wind Nowait 'Importing A/C Outstandings...'+Chr(13)+"Transfer Customer O/S"
sele sele(1)
...
sele sele(1)
use &datatransferpath\doc_header alias fdet
index on doc_id to fdet
sele sele(1)
use &datatransferpath\daybook alias fout
sele mslip
go top
vou =voucher  + 10
svou=svoucher + 10
sele fout
set filter to type='D' and (amount#adjusted)
go top
scan 
	select fdet
	seek fout.vid
	mvcn=alltr(fdet.tseries)+'/'+alltr(str(fdet.tno))
	sele mord
	seek STR(fout.acccode,6)
	sele mpend
	appen blan
	repl acgroup with mord->scode
	repl ord with str(fout->acccode,6)
	repl ddate with fdet->doc_date
	replace duedate with fdet->doc_duedat
	repl vcn with mvcn
	repl invtype with 'O'
	repl voucher  with vou
	repl svoucher with svou
	repl final with abs(fout->amount)&&iif(acgroup='D31',-abs(fout->amount),abs(fout->amount))
	repl itemope with abs(fout->adjusted)&&iif(acgroup='D31',-abs(fout->adjusted),abs(fout->adjusted))
	repl balance with (abs(fout.amount)-abs(fout->adjusted))
	repl name with vcn
	vou=vou+5
	svou=svou+5
endsc
...
Wait Wind Nowait 'Importing A/C Outstandings...'+Chr(13)+"Transfer Supplier O/S"
sele sele(1)
use &datdev\pendings.&datext alias mpend
sele sele(1)
use &datdev\order.&datext alias mord
...
svou=svoucher + 10
sele fout
set filter to type='C' and empty(contcode) and empty(vtype) and (amount#adjusted)
go top
scan 
	select fdet
	seek fout.vid
...
Wait Wind Nowait 'Importing Ledger Balance...'+Chr(13)+"Transfer Ledger Balance"
	sele sele(1)
	use &datdev\order.&datext alias mord
	inde on ordno to mord 
